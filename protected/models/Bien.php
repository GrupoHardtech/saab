...
class Bien extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			...
			array('BIE_stockActual,BIE_precio, BIE_stockMinimo, IDCATALOGO', 'numerical', 'integerOnly'=>true),
			array('BIE_caracteristica, BIE_marca', 'length', 'max'=>150),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('IDBIEN, BIE_stockActual, BIE_stockMinimo, BIE_caracteristica, BIE_marca, IDCATALOGO, BIE_precio', 'safe', 'on'=>'search'),
		);
	}
	...
	public function search()
	{
		...
		$criteria->compare('IDCATALOGO',$this->IDCATALOGO);
                $criteria->compare('BIE_precio',$this->BIE_precio);
